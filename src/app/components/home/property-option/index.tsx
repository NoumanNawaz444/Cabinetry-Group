...
export default function DiscoverProperties() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch('/api/servicedata'); // updated endpoint
                if (!res.ok) throw new Error('Failed to fetch');

                const data = await res.json();

                // Map categories to avoid duplicates
                ...
                setServicesData(uniqueCategoryData); // renamed for services
            } catch (error) {
    ...
    return (
        <section className='dark:bg-darkmode'>
            <div className="container lg:max-w-screen-xl md:max-w-screen-md mx-auto px-4">
                <h2 className="text-4xl font-bold mb-12 text-midnight_text dark:text-white" data-aos="fade-left">Discover Services</h2>
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 lg:gap-4 gap-8">
                    {servicesData.map((service, index) => (
                        <div
                            ...
                            onClick={() => updateFilter('category', service.category)}
                            data-aos="fade-up"
                            data-aos-delay={`${index * 100}`}
                        >
                            <Link href={`/properties/properties-list`} className="group">
                                <Image
                                    src={service.category_img}
                                ...
                                <p className="text-base text-gray">{service.count} Services</p>
                            </Link>
                        </div>
                    ))}
