...
interface PropertyContextType {
  ...
  setProperties: Dispatch<SetStateAction<ServiceData[]>>;
  filters: Filters;
  setFilters: any;
...
export const AppContextProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [allProperties, setAllProperties] = useState<ServiceData[]>([]);
  const [properties, setProperties] = useState<ServiceData[]>([]);
  ...
  useEffect(() => {
    const filteredProperties = allProperties.filter((property) => {
      return (
        (!filters.keyword || property.service_title.toLowerCase().includes(filters.keyword.toLowerCase())) &&
